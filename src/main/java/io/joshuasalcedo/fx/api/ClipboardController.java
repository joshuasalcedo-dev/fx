...
public class ClipboardController {
  ...
  @ApiResponses(
      value = {
        @ApiResponse(
            responseCode = "200",
            description = "Successfully retrieved clipboard entries",
            content =
                @Content(
  ...
  public ResponseEntity<Page<ClipboardDto>> clipboards(
      ...
      @Parameter(description = "Number of items per page (max 100)", example = "20")
          @RequestParam(name = "max", defaultValue = "20")
          @Min(1)
          int size) {
  ...
  @Operation(
      ...
      description = "Returns a list of all pinned clipboard entries")
  @ApiResponses(
      value = {
        @ApiResponse(
            responseCode = "200",
            description = "Successfully retrieved pinned clipboard entries",
            content =
                @Content(
                    mediaType = "application/json",
  ...
  public ResponseEntity<List<ClipboardDto>> pinnedClipboards() {
    ...
    List<ClipboardDto> dtos = pinnedEntries.stream().map(this::toDto).collect(Collectors.toList());
  ...
  @Operation(
      ...
      description = "Toggles the pin status of a clipboard entry")
  @ApiResponses(
      value = {
        @ApiResponse(
            responseCode = "200",
            description = "Successfully toggled pin status",
            content =
                @Content(
                    mediaType = "application/json",
                    schema = @Schema(implementation = ClipboardDto.class))),
    ...
    return clipboardService
        ...
        .map(
            entry -> {
              ...
              return clipboardService
                  .setPinned(request.id(), newPinStatus)
  ...
  @Operation(
      ...
      description = "Deletes all clipboard entries, optionally including pinned entries")
  @ApiResponses(
      value = {
  ...
  @Operation(
      ...
      description = "Deletes a specific clipboard entry by ID")
  @ApiResponses(
      value = {
        @ApiResponse(
            responseCode = "204",
            description = "Successfully deleted the clipboard entry"),
        @ApiResponse(responseCode = "404", description = "Clipboard entry not found")
      })
  ...
  public ResponseEntity<StopClipboardResponse> stopClipboard() {
    ...
    if (stopped) {
      return ResponseEntity.ok(
          new StopClipboardResponse(true, "Clipboard monitoring stopped successfully"));
    } else {
  ...
  @Operation(
      ...
      description = "Searches clipboard entries by content")
  @ApiResponses(
      value = {
        @ApiResponse(
            responseCode = "200",
  ...
  public ResponseEntity<List<ClipboardDto>> getRecentClipboards(
    ...
    List<ClipboardDto> dtos = recentEntries.stream().map(this::toDto).collect(Collectors.toList());
  ...
  private ClipboardDto toDto(ClipboardEntry entry) {
    return new ClipboardDto(
        entry.getId(), entry.getTimestamp(), entry.getContent(), entry.isPinned());
  }
