...
class ClipboardServiceTest {
  ...
  @Mock private ClipboardRepository clipboardRepository;

  private ClipboardService clipboardService;

  @Mock private ClipboardWebSocketController clipboardWebSocketController;

  @Mock private ClipboardMonitor clipboardMonitor;
  ...
  void save_ValidContent_SavesNewEntry() {
    ...
    when(clipboardRepository.findFirstByContentAndTimestampAfter(
  ...
  void save_DuplicateContent_UpdatesExistingEntry() {
    ...
    ClipboardEntry existingEntry =
        ClipboardEntry.builder().id(1L).content(content).timestamp(oldTimestamp).build();
  ...
  void setPinned_ExistingEntry_UpdatesPinnedStatus() {
    ...
    ClipboardEntry entry =
        ClipboardEntry.builder().id(id).content("Test content").isPinned(false).build();
  ...
  void unpin_CallsSetPinnedWithFalse() {
    ...
    Long id = 1L;
    ClipboardEntry entry =
        ClipboardEntry.builder().id(id).content("Test content").isPinned(true).build();
  ...
  void findById_ExistingEntry_ReturnsEntry() {
    // Arrange
    Long id = 1L;
    ClipboardEntry entry = ClipboardEntry.builder().id(id).content("Test content").build();
  ...
  void findAllPinned_ReturnsPinnedEntries() {
    // Arrange
    List<ClipboardEntry> pinnedEntries =
        List.of(
  ...
  void searchByContent_ValidTerm_ReturnsMatchingEntries() {
    ...
    when(clipboardRepository.findByContentContainingIgnoreCase(searchTerm, pageable))
        .thenReturn(page);
  ...
  void searchByContent_EmptyTerm_ReturnsEmptyPage() {
    ...
    verify(clipboardRepository, never())
        .findByContentContainingIgnoreCase(anyString(), any(Pageable.class));
  }
  ...
  void findRecent_ReturnsEntriesFromLastNHours() {
    // Arrange
    int hours = 24;
    List<ClipboardEntry> recentEntries =
        List.of(
    ...
    assertTrue(
        Math.abs(threshold.until(expectedThreshold, java.time.temporal.ChronoUnit.SECONDS)) < 5);
  }
  ...
  void deleteOlderThan_IncludePinnedTrue_DeletesAllOldEntries() {
    ...
    verify(clipboardRepository, never())
        .deleteByTimestampBeforeAndIsPinnedFalse(any(LocalDateTime.class));
  }
  ...
  void deleteOlderThan_IncludePinnedFalse_DeletesOnlyUnpinnedOldEntries() {
    // Arrange
    int hours = 48;
    when(clipboardRepository.deleteByTimestampBeforeAndIsPinnedFalse(any(LocalDateTime.class)))
        .thenReturn(3);
